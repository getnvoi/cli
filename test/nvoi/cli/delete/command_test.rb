...
class DeleteCommandTest < Minitest::Test
  def test_command_initializes
    ...
    command = Nvoi::Cli::Delete::Command.new(options)

    assert_instance_of Nvoi::Cli::Delete::Command, command
  end

  def test_resolve_config_path_default
    command = Nvoi::Cli::Delete::Command.new({ config: nil, dir: nil })
  ...
  def test_resolve_config_path_with_dir
    command = Nvoi::Cli::Delete::Command.new({ config: nil, dir: "/app" })
  ...
  def test_resolve_config_path_explicit
    command = Nvoi::Cli::Delete::Command.new({ config: "/custom/config.enc", dir: "/app" })
  ...
  def test_resolve_config_path_with_current_dir
    command = Nvoi::Cli::Delete::Command.new({ config: "deploy.enc", dir: "." })
