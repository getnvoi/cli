  ...
  module Objects
      ...
      private
        ...
        def validate_service_server_bindings
          ...
          app.app.each do |svc_name, svc_config|
            raise ConfigValidationError, "app.#{svc_name}: servers field is required" if svc_config.servers.empty?

            svc_config.servers.each do |server_ref|
              raise ConfigValidationError, "app.#{svc_name}: references undefined server '#{server_ref}'" unless defined_servers[server_ref]
            end
          end
          ...
          app.services.each do |svc_name, svc_config|
            raise ConfigValidationError, "services.#{svc_name}: servers field is required" if svc_config.servers.empty?

            svc_config.servers.each do |server_ref|
              raise ConfigValidationError, "services.#{svc_name}: references undefined server '#{server_ref}'" unless defined_servers[server_ref]
            end
          end
        end
    ...
    class Application
      ...
      def initialize(data = {})
        ...
        @servers = (data["servers"] || {}).transform_values { |v| ServerConfig.new(v || {}) }
        @app = (data["app"] || {}).transform_values { |v| AppServiceConfig.new(v || {}) }
        @database = data["database"] ? DatabaseConfig.new(data["database"]) : nil
        @services = (data["services"] || {}).transform_values { |v| ServiceConfig.new(v || {}) }
        @env = data["env"] || {}
    ...
    class ServerConfig
      ...
      def initialize(data = {})
        ...
        @volumes = (data["volumes"] || {}).transform_values { |v| ServerVolumeConfig.new(v || {}) }
      end
    end
