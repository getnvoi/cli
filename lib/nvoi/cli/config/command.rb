    ...
    module Config
      ...
      class Command
        ...
        def init(name, environment)
          result = ConfigApi.init(name: name, environment: environment)
        ...
        def domain_set(provider, api_token:, account_id:)
          with_config do |data|
            ConfigApi.set_domain_provider(data, provider: provider, api_token: api_token, account_id: account_id)
          end
        end
        ...
        def provider_set(provider, **opts)
          with_config do |data|
            ConfigApi.set_compute_provider(data, provider: provider, **opts)
          end
        end
        ...
        def server_set(name, master: false, type: nil, location: nil, count: 1)
          with_config do |data|
            ConfigApi.set_server(data, name: name, master: master, type: type, location: location, count: count)
          end
        end

        def server_rm(name)
          with_config do |data|
            ConfigApi.delete_server(data, name: name)
          end
        end
        ...
        def volume_set(server, name, size: 10)
          with_config do |data|
            ConfigApi.set_volume(data, server: server, name: name, size: size)
          end
        end

        def volume_rm(server, name)
          with_config do |data|
            ConfigApi.delete_volume(data, server: server, name: name)
          end
        end
        ...
        def app_set(name, servers:, **opts)
          with_config do |data|
            ConfigApi.set_app(data, name: name, servers: servers, **opts)
          end
        end

        def app_rm(name)
          with_config do |data|
            ConfigApi.delete_app(data, name: name)
          end
        end
        ...
        def database_set(servers:, adapter:, **opts)
          with_config do |data|
            ConfigApi.set_database(data, servers: servers, adapter: adapter, **opts)
          end
        end
        ...
        def service_set(name, servers:, image:, **opts)
          with_config do |data|
            ConfigApi.set_service(data, name: name, servers: servers, image: image, **opts)
          end
        end

        def service_rm(name)
          with_config do |data|
            ConfigApi.delete_service(data, name: name)
          end
        end
        ...
        def secret_set(key_name, value)
          with_config do |data|
            ConfigApi.set_secret(data, key: key_name, value: value)
          end
        end
        ...
        def env_set(key_name, value)
          with_config do |data|
            ConfigApi.set_env(data, key: key_name, value: value)
          end
        end
